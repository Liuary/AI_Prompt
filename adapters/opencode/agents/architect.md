...
3. **读取模型配置**：读取 .ai/config.yaml 的 models 节，按以下优先级匹配当前 Agent 的模型后端：
   ...
   - 匹配结果中的 provider、model_name、ase_url、pi_key_env 字段用于配置模型连接。
...
自动闭环默认关闭。只有当子计划 `status.md` 同时满足以下条件时，才允许推进下游阶段：
...
1. 读取 `.ai/plan/deps.yaml` 获取各阶段的依赖关系，计算当前可推进的阶段集合：
   ...
   - `type: soft` 依赖未满足的阶段标记警告但仍可推进
   - `type: mutual_exclusion` 的阶段严格按拓扑顺序串行
2. 向用户展示可推进的阶段列表，由用户手动选择并启动对应 Agent（如 `code`）。
3. 各阶段 Agent 可通过 `task` 工具调度对应子 Agent（如 `code-worker`），完成子计划后更新 `status.md`。
4. 任一阶段完成后，Architect 检查该阶段的依赖列表：
   - 将下游阶段 `status.md` 中的 `依赖状态` 更新为 `satisfied`
   - 若下游阶段所有 hard 依赖均已满足，告知用户可推进下一阶段

### 停止条件

遇到计划外架构变更、权限不明、连续两次验收失败或测试环境缺失，必须调用 `load skill update-stage-status` 将状态改为 `paused`，当前责任 Agent 改为 `user`。

若当前 Agent 无法继续推进，告知用户下一步应启动哪个 Agent。
