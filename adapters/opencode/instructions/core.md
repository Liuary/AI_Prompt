...
> **OpenCode 平台工具**：OpenCode 提供了 Todo 工具（任务跟踪与管理）、提问工具（向用户确认与收集信息）等各类内置工具。Agent 应主动关注并充分利用这些平台工具辅助工作，而非仅依赖 bash/read/grep 等基础工具完成任务。
...
自动推进规则：
- 默认值来自 `.ai/config.yaml` defaults 字段（部署模板默认为 `manual + disabled`，仓库自身为 `auto + enabled` 以支持自驱动）。仅当 `执行模式=auto` 且 `自动推进=enabled` 时，允许 Architect 启动 AutoRunner 或 隔离 worktree session。
...
- Code、CodeWorker、ReviewWorker、TestWriter、Tester 不得自行创建新的隔离 worktree；只更新状态并将控制权交回 AutoRunner 或用户。
- Debug 不得启动其他 Agent。
