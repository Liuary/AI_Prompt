...
> 制定时间：2026-05-13
> 版本：v2.0（五阶段）+ v3.0（三阶段）扩展
> 状态：stage-09 ready_for_review（v3.0 收关阶段）
...
- **规则校验器**：冲突检测、冗余检测、循环规则检测
- **知识库自动化**：会话结束时自动将经验写入 kb/
- **跨会话记忆**：基于 dev_last.md，启动时自动恢复上下文

### 阶段二：多人协作基础

> 优先级：🟡  |  状态：review_passed  |  详见：stage-02/

- **任务归属**：current.md 格式增强，Agent 接手工作前声明模块
- **冲突检测**：多人/多Agent 修改同一文件前的预警
- **进度同步**：sync_status Skill，聚合所有成员任务视图
- **跨项目复刻**：namespace 字段预留，不实现
...
> 优先级：⚪  |  状态：review_passed  |  详见：stage-04/

- **规范文档**：specs/ 目录，5 篇核心规范文档
- **模板市场储备**：打包格式设计 + 示例模板

### 阶段五：并行支持改造

> 优先级：🟡  |  状态：review_passed  |  详见：stage-05/
...
### 阶段九：AI 驻留能力 / Claudian 方向（v3.0 收关）

> 优先级：⚪  |  状态：ready_for_review  |  详见：stage-09/

- **Obsidian Vault 模板**：将 .ai/ 工作区变为可视化 Vault，支持 wikilink 双向链接和图表视图
...
| # | 目标 | 优先级 | 一句话描述 |
|---|------|--------|-----------|
| 6 | **向量化知识库** | 🔴 最高 | `.ai/kb/` + 可选语义索引层，文件系统仍是主存储 |
...
- 人工控制点不丢失：语义检索是辅助决策，最终审批仍保留在人工流程
- 声明式优于程序式：新增配置以 YAML/Markdown 声明式文件驱动

### 阶段六：向量化知识库

> 优先级：🔴  |  状态：ready_for_code  |  详见：stage-06/
...
> 优先级：🟠  |  状态：ready_for_code  |  详见：stage-08/
